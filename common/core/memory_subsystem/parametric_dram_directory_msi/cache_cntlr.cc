...
VERI_LOG("----------------------------------------------");
VERI_LOG("%c%c %x +%u..+%u", mem_op_type == Core::WRITE ? 'W' : 'R', mem_op_type == Core::READ_EX ? 'X' : ' ', ca_address, offset, data_length);
...
//-------------------NOW OUR CASE STARTS
   if (cache_hit)
   {
	...
	VERI_LOG("processMemOpFromCore l%d name=%s beforecall processShmemReqFromPrevCache nextcntlr", m_mem_component,MemComponentString(m_mem_component));
	...
	VERI_LOG("processMemOpFromCore l%d name=%s after call to processShmemReqFromPrevCache next hit = %d", m_mem_component,MemComponentString(m_mem_component),next_cache_hit);
...
*/
   ...
   VERI_LOG("returning %s, latency %llu ns", HitWhereString(hit_where), total_latency.getNS());
   return hit_where;
}
...
VERI_LOG("copyDataFromNextLevel l%d=%s", m_mem_component,MemComponentString(m_mem_component));
   ...
   if (cache_block_info)
   {
      ...
      VERI_LOG("copyDataFromNextLevel l%d=%s done (updated)", m_mem_component,MemComponentString(m_mem_component));
   }
   else
   {
      // Insert the Cache Block in our own cache
      insertCacheBlock(address, cstate, data_buf, m_core_id, ShmemPerfModel::_USER_THREAD);
      VERI_LOG("copyDataFromNextLevel l%d=%s done (inserted)", m_mem_component,MemComponentString(m_mem_component));
   }
}
...
CacheCntlr::doPrefetch(IntPtr prefetch_address, SubsecondTime t_start)
{
   ...
   VERI_LOG("prefetching %x", prefetch_address);
...
CacheCntlr::processShmemReqFromPrevCache(CacheCntlr* requester, Core::mem_op_t mem_op_type, IntPtr address, bool modeled, bool count, Prefetch::prefetch_type_t isPrefetch, SubsecondTime t_issue, bool have_write_lock)
{
   ...
   bool cache_hit = operationPermissibleinFlexCache(address, mem_op_type,isShared(m_core_id),true), sibling_hit = false, prefetch_hit = false;
	...
	{
		VERI_LOG("miss in mem=%s for addr=(0x%x) isprefetch:%d",MemComponentString(m_mem_component),address,isPrefetch);
	}
   ...
   else // !cache_hit: either data is not here, or operation on data is not permitted
   {
      ...
      else // last-level cache
      {
         if (cache_block_info && cache_block_info->getCState() == CacheState::EXCLUSIVE)
         {
            ...
            cache_hit = true;
            hit_where = HitWhere::where_t(m_mem_component);
            VERI_LOG("Silent upgrade from E -> M for address %x mem=%s", address,MemComponentString(m_mem_component));
            cache_block_info->setCState(CacheState::MODIFIED);
         }
   ...
   VERI_LOG("returning %s", HitWhereString(hit_where));
   return hit_where;
}
...
CacheCntlr::notifyPrevLevelEvict(core_id_t core_id, MemComponent::component_t mem_component, IntPtr address)
{
 ...
 VERI_LOG("@%x", address);
  ...
  if (m_master->m_evicting_buf && address == m_master->m_evicting_address) 
   {
		VERI_LOG("here being evicted mem=%s",MemComponentString(m_mem_component));
   } 
  else 
  {
      #ifdef ENABLE_TRACK_SHARING_PREVCACHES
	      SharedCacheBlockInfo* cache_block_info = getCacheBlockInfo(address);
	      VERI_LOG("here in state %c mem=%s ", CStateString(getCacheState(address)),MemComponentString(m_mem_component));
	      assert(cache_block_info);
	      PrevCacheIndex idx = m_master->m_prev_cache_cntlrs.find(core_id, mem_component);
...
CacheCntlr::initiateDirectoryAccess(Core::mem_op_t mem_op_type, IntPtr address, bool isPrefetch, SubsecondTime t_issue)
{
   ...
   else
   {
      ...
      VERI_LOG("%u previous waiters", m_master->m_directory_waiters.size(address));
   }
}
...
CacheCntlr::processExReqToDirectory(IntPtr address)
{
   ...
   VERI_LOG("EX REQ>%d @ %x", getHome(address) ,address);
...
//   VERI_LOG("OPpermissible address %x state %c: permissible %d", address, CStateString(cstate), cache_hit);
   return cache_hit;
}
//------------------------------PRAK-LOG------------------------------------------------------------------------
bool 
CacheCntlr::operationPermissibleinFlexCache(IntPtr address, Core::mem_op_t mem_op_type,bool isShared,bool isFlexOp,
CacheBlockInfo **cache_block_info)
{
...
//   VERI_LOG("OPpermissibleflex address %x state %c: permissible %d", address, CStateString(cstate), cache_hit);
   return cache_hit;
...
CacheCntlr::getCacheBlockInfo(IntPtr address,bool print)
{
	SharedCacheBlockInfo* info;
	if(print)
	{
		info=(SharedCacheBlockInfo*) m_master->m_cache->peekSingleLine(address,true,false);
	}
	else
	{
		info=(SharedCacheBlockInfo*) m_master->m_cache->peekSingleLine(address);
	}
   return info;
}
...
//--------------------------PRAK-LOG----------------------------------------------------
SharedCacheBlockInfo*
CacheCntlr::getFlexCacheBlockInfo(IntPtr address,bool isShared,bool isFlexOp)
{
	return (SharedCacheBlockInfo*) m_master->m_cache->peekSingleLine(address,true,true);	
}
//--------------------------------------------------------------------------------------

CacheState::cstate_t
CacheCntlr::getCacheState(IntPtr address,bool print)
{
	
   SharedCacheBlockInfo* cache_block_info;
	if(print)
	{
		cache_block_info = getCacheBlockInfo(address,print);
	}
	else
	{
...
CacheCntlr::invalidateCacheBlock(IntPtr address)
{
   ...
   m_master->m_cache->invalidateSingleLine(address);
	VERI_LOG("in invalidatecacheblk mem=%s ",MemComponentString(m_mem_component));

   if (m_next_cache_cntlr)
  {
      VERI_LOG("calling notifyevict from mem=%s to %s ",MemComponentString(m_mem_component),MemComponentString(m_next_cache_cntlr->getMemComponent()));
		
      m_next_cache_cntlr->notifyPrevLevelEvict(m_core_id_master, m_mem_component, address);
  }
   VERI_LOG("%x %c > %c mem=%s", address, CStateString(old_cstate), CStateString(getCacheState(address)),MemComponentString(m_mem_component));
}
...
//-----------------------------------------------------------------
	...
	if(isShared)
	{
		LOG_ASSERT_ERROR(getCacheState(address,true) == CacheState::INVALID, "we already have this line in flex, can't add it again");
	}
	else
	{
   ...
   if (m_next_cache_cntlr && !m_perfect)
	...
	VERI_LOG("insertCacheBlock l%d name=%s local done", m_mem_component,MemComponentString(m_mem_component));
...
VERI_LOG("evict INV %x", evict_address);
      ...
      VERI_LOG("insertCacheBlock =%s evict done", MemComponentString(m_mem_component));
   }

   VERI_LOG("insertCacheBlock %s end",MemComponentString(m_mem_component));
   return cache_block_info;
}
...
{
   VERI_LOG("updateCacheBlock mem=%s",MemComponentString(m_mem_component));
   ...
   VERI_LOG("@%x  %c > %c (req: %c)", address, CStateString(old_cstate),
...
VERI_LOG(" ");
   ...
   if (m_master->m_evicting_buf && (address == m_master->m_evicting_address)) {
      VERI_LOG("writing to evict buffer %x mem=%s", address,MemComponentString(m_mem_component));
assert(offset==0);
...
VERI_LOG("WB REQ<%u @ %x", sender, address);
         ...
         || (shmem_msg_type == PrL1PrL2DramDirectoryMSI::ShmemMsg::UPGRADE_REP) )
   {
      ...
      while(! m_master->m_directory_waiters.empty(address)) {
         ...
         {
            VERI_LOG("have SHARED, upgrading to MODIFIED for #%u", request->cache_cntlr->m_core_id);
         ...
         if (t_here > t_core) {
VERI_LOG("adjusting time in #%u from %llu to %llu", request->cache_cntlr->m_core_id, t_core.getNS(), t_here.getNS());
...
VERI_LOG("wakeup user #%u", request->cache_cntlr->m_core_id);
         ...
         getLock().acquire();
         VERI_LOG("about to dequeue request (%p) for address %x", m_master->m_directory_waiters.front(address), address );
         m_master->m_directory_waiters.dequeue(address);
         delete request;
      }
      getLock().release();
VERI_LOG("woke up all");
   }
...
VERI_LOG("processExRepFromDramDirectory l%d", m_mem_component);
...
VERI_LOG("processExRepFromDramDirectory l%d end", m_mem_component);
}
...
CacheCntlr::processShRepFromDramDirectory(core_id_t sender, core_id_t requester, PrL1PrL2DramDirectoryMSI::ShmemMsg* shmem_msg)
{
   // Forward data from message to LLC, don't incur LLC data access time (writeback will be done asynchronously)
   //getMemoryManager()->incrElapsedTime(m_mem_component, CachePerfModel::ACCESS_CACHE_DATA_AND_TAGS);
VERI_LOG("processShRepFromDramDirectory l%d", m_mem_component);
...
VERI_LOG("processShRepFromDramDirectory l%d", m_mem_component);
   ...
   {
      LOG_PRINT_ERROR("Trying to upgrade a block that is not SHARED_UPGRADING but %c (%x)",CStateString(cstate), address);
   }
}
...
{
   IntPtr address = shmem_msg->getAddress();
VERI_LOG("processInvReqFromDramDirectory l%d", m_mem_component);
   ...
   if (cstate != CacheState::INVALID)
   {
      ...
      {
        VERI_LOG("invalidating something else than SHARED: %c ", CStateString(cstate));
      }
      //assert(cstate == CacheState::SHARED);
   ...
   else
   {
      // Update Shared Mem perf counters for access to L2 Cache
      getMemoryManager()->incrElapsedTime(m_mem_component, CachePerfModel::ACCESS_CACHE_TAGS, ShmemPerfModel::_SIM_THREAD);
VERI_LOG("invalid @ %x, hoping eviction message is underway", address);
   }
}
...
{
   IntPtr address = shmem_msg->getAddress();
VERI_LOG("processFlushReqFromDramDirectory l%d", m_mem_component);
...
{
   IntPtr address = shmem_msg->getAddress();
VERI_LOG("processWbReqFromDramDirectory l%d", m_mem_component);
...
VERI_LOG("block transfer called for mod=%d min_way=%d max_way=%d",module_index,min_way,max_way);
	for(UInt32 s=set_si;s<set_fi;s++)
	{
		...
		{
			CacheBlockInfo* cache_block_info=m_master->m_cache->peekBlock(s,v);
			if(cache_block_info==NULL)
			{
				continue;
			}
			...
			{
				//invalidate the block in dead way
				cache_block_info->invalidate();
			}			
		}
	}
...
{
//VERI_LOG("cache lock acquire %u # %u @ %lx", m_mem_component, m_core_id, address);
   assert(isFirstLevel());
...
{
//VERI_LOG("cache lock release %u # %u @ %lx", m_mem_component, m_core_id, address);
   assert(isFirstLevel());
...
{
//VERI_LOG("stack lock acquire %u # %u @ %lx", m_mem_component, m_core_id, address);
...
{
//VERI_LOG("stack lock release %u # %u @ %lx", m_mem_component, m_core_id, address);
   if (this_is_locked)
